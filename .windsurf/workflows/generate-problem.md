...
- 每个 `solution`：
  ...
  - **简洁性（重要）**：`steps` 要精炼、不冗余。
    ...
    - **避免步骤间重复**：不要重复在之前步骤中已经建立的信息。如果步骤 1（分析）已经陈述了一个事实（例如"从 A 到 B 跨越 3 个单位"），后续步骤应在此基础上推进，而不要重述该事实。每个步骤应贡献新信息或推进推理，而非复述已说过的内容。
  ...
  - **避免场景间重复**：不要添加重复其他场景已展示信息的场景。例如，如果 `number-line` 已经清晰显示了最终答案（如"现在儿子: 28"、"现在父亲: 53"），就不要再添加 `result-badges` 场景重复相同信息。每个场景应提供新的视觉视角或尚未传达的信息。
- 解法应真正解决题目，不能绕过关键推理。
