...
4. **逐项诊断**，按以下维度列出问题清单（带证据引用原文片段）：
   ...
   - **figures 泄露**：`figures[].svg` / `alt` 是否出现方法暗示（捆绑框、∧ 形空位、隔板①②、"只能填…"、"固定一人"、"每格 = 左 + 下" 等）。若 SVG 本身优秀但有方法暗示，标记为"应移入 `solutions[i].scenes`"。
   ...
   - **解法冗余（重要）**：
     ...
     - **避免步骤间重复**：是否重复了在之前步骤中已经建立的信息。如果步骤 1（分析）已经陈述了一个事实（例如"从 A 到 B 跨越 3 个单位"），后续步骤应在此基础上推进，而不要重述该事实。每个步骤应贡献新信息或推进推理，而非复述已说过的内容。
     - **奥卡姆剃刀原则**：对于非常简单的题目，是否使用了不必要的解释性前缀（如"观察"、"分析"）。若计算从题面显而易见，应直接展示计算，无需前缀。
   - **场景冗余**：
     - 是否添加了重复其他场景已展示信息的场景。例如，如果 `number-line` 已经清晰显示了最终答案（如"现在儿子: 28"、"现在父亲: 53"），是否还添加了 `result-badges` 场景重复相同信息。每个场景应提供新的视觉视角或尚未传达的信息。
   - **variant**：
...
3. **把泄露方法的 figure 搬家**：若原 `figures[]` 中某张 SVG 只有在知道方法后才画得出，将其以 `{ "kind": "svg", "svg": "…" }` 放入对应 `solutions[i].scenes`（通常接在该方法的分析/建模步骤之后），并在 `figures[]` 留一张中性示意图。
