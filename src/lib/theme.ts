...
const getStoredTheme = (): Theme | null => {
  return getStorageItem<Theme | null>("theme", null);
};
...
export const themeScript = `(function(){try{var s=localStorage.getItem("theme");var t=s?JSON.parse(s):null;if(t){document.documentElement.classList.toggle("dark",t.mode==="dark");document.documentElement.setAttribute("data-color-scheme",t.colorScheme);if(t.customColors)for(var k in t.customColors)document.documentElement.style.setProperty("--custom-"+k,t.customColors[k]);}else{document.documentElement.setAttribute("data-color-scheme","default");var d=window.matchMedia("(prefers-color-scheme: dark)").matches;if(d)document.documentElement.classList.add("dark");}}catch{}})();`;
