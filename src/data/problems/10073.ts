...
export default {
  ...
  "module": "杂题",
  "difficulty": "进阶",
  "question": "一批工件共 5 件（A、B、C、D、E），每件都要先在机床甲上加工，再到机床乙上加工（顺序不能颠倒；同一机床一次只能加工一件）。各工件在两台机床上需要的时间（单位：小时）见下表。请合理安排 5 件工件在机床甲上的加工顺序，使全部完成的总工时最短。最短总工时是多少？",
  "figures": [
    {
      "svg": svg1,
  ...
  "solutions": [
    {
      ...
      "label": "约翰逊规则（分组排序）",
      "steps": [
        "约翰逊规则：甲≤乙的A,C按甲升序(A→C)；甲>乙的B,D,E按乙降序(E→D→B)。总序A→C→E→D→B，乙在27小时完工。"
      ],
      "scenes": [
        {
          "kind": "equation-list",
