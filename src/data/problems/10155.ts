import type { ProblemData } from "@/types/problem";

export default {
  id: "10155",
  title: "男女排队计数·逆向思维",
  grade: "五年级",
  module: "计数",
  difficulty: "挑战",
  question:
    "20个女生面向右排成一排。4个男生插入队伍中，面向左。每个男生数自己前面有多少个女生，得到的数字分别是3、6、15、18。现在每个女生也数自己前面有多少个男生。求所有女生数的数字之和是多少？",
  solutions: [
    {
      key: "reverse_counting",
      label: "逆向计数法",
      steps: [
        "分析：男生面向左，数前面的女生（即右边的女生）；女生面向右，数前面的男生（即左边的男生）。",
        "男生看到的女生数分别是3、6、15、18，说明从右往左数，男生分别位于第4、7、16、19个位置（因为前面分别有3、6、15、18个女生）。",
        "换个角度：女生数左边的男生，相当于从左往右看，每个男生右边有多少个女生。",
        "总配对数 = 男生数 × 女生数 = 4 × 20 = 80。",
        "男生看到的女生数之和 = 3 + 6 + 15 + 18 = 42，这是男生右边女生数的和。",
        "女生看到的男生数之和 = 80 − 42 = 38。",
      ],
      scenes: [
        {
          kind: "equation-list",
          rows: [
            {
              lhs: "男生看到的女生数",
              rhs: "3 + 6 + 15 + 18 = 42",
              note: "男生右边女生数的和",
            },
            {
              lhs: "总配对数",
              rhs: "4 × 20 = 80",
              note: "每个男生与每个女生形成一对",
            },
            {
              lhs: "女生看到的男生数",
              rhs: "80 − 42 = 38",
              badge: "结论",
              status: "keep",
            },
          ],
          caption: "用总配对数减去男生看到的，得到女生看到的",
        },
        {
          kind: "result-badges",
          items: [
            {
              icon: "🔢",
              count: 38,
              label: "所有女生数的数字之和",
            },
          ],
          caption: "38",
        },
      ],
    },
    {
      key: "pairing_analysis",
      label: "配对分析法",
      steps: [
        "分析：把每个男生和每个女生看作一对，总共有4 × 20 = 80对。",
        "对于每一对（男，女），要么是男在女左边（女生能看到这个男生），要么是男在女右边（男生能看到这个女生）。",
        "男生看到的女生数之和 = 42，说明有42对是男在女右边。",
        "剩下的80 − 42 = 38对是男在女左边，这就是女生看到的男生数之和。",
      ],
      scenes: [
        {
          kind: "compare-bars",
          rows: [
            {
              label: "总配对数",
              value: 80,
              max: 80,
              tone: "primary",
            },
            {
              label: "男在女右边",
              value: 42,
              max: 80,
              tone: "muted",
            },
            {
              label: "男在女左边",
              value: 38,
              max: 80,
              tone: "primary",
              marker: true,
            },
          ],
          caption: "总配对数 = 男在女右边 + 男在女左边",
        },
      ],
    },
  ],
  variant: {
    question:
      "15个女生面向右排成一排。3个男生插入队伍中，面向左。每个男生数自己前面有多少个女生，得到的数字分别是2、8、12。求所有女生数的数字之和是多少？",
    fields: [
      {
        key: "sum",
        label: "女生数的数字之和",
      },
    ],
    answer: {
      sum: 25,
    },
    hint: "总配对数 = 3 × 15 = 45。男生看到的和 = 2 + 8 + 12 = 22。女生看到的和 = 45 − 22 = 23？不对，再算一下：45 − 22 = 23。",
  },
  knowledgePoints: [
    {
      slug: "reverse-reasoning",
      name: "逆向推理",
      summary: "从结果倒着走回起点：每一步都反着做那一步的运算。",
    },
    {
      slug: "correspondence",
      name: "对应思想",
      summary: "盈亏 / 归一 / 分配等题里，把“每份多少”与“份数”一对一挂钩，让两组条件作差或作比。",
    },
  ],
  tags: ["逆向推理", "对应思想"],
} satisfies ProblemData;
