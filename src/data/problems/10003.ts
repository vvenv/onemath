  ...
  "solutions": [
    {
      ...
      "label": "线段图法",
      "steps": [
        "画一条线段，左端代表过去儿子 3 岁时，右端代表未来父亲 78 岁时。",
        "线段上取三个关键点：过去父亲年龄（=现在儿子年龄）、现在儿子年龄、现在父亲年龄、未来儿子年龄（=现在父亲年龄）。",
        "观察发现，父子年龄差是不变的。将年龄差设为一段。",
        "从 3 岁到 78 岁，包含了三个年龄差：儿子过去到现在的差、儿子现在到父亲现在的差、父亲现在到未来的差。",
        "年龄差 = (78 − 3) ÷ 3 = 25 岁。",
        "儿子现在：3 + 25 = 28 岁。",
        "父亲现在：28 + 25 = 53 岁。"
      ],
    ...
    },
    {
      ...
      "label": "方程法",
      "steps": [
        "设现在儿子年龄为 x，父亲年龄为 y，年龄差 d = y − x。",
        "「父亲像儿子时」：x − d = 3（过去儿子 3 岁）。",
        "「儿子像父亲时」：y + d = 78（未来父亲 78 岁）。",
        "两式相减：(y + d) − (x − d) = 78 − 3 ⇒ (y − x) + 2d = 75。",
        "因为 y − x = d，所以 d + 2d = 75 ⇒ d = 25。",
        "代入得 x = 3 + 25 = 28，y = 78 − 25 = 53。"
      ],
      "scenes": [
        {
          "kind": "equation-list",
