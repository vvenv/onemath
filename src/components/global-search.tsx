...
function scoreProblem(p: ProblemIndexEntry, q: string): number {
  if (!q) return 0;
  ...
  if (p.questionSnippet.toLowerCase().includes(q)) return 100;
  return 0;
}
...
export function GlobalSearch() {
  ...
  const results = useMemo(() => {
    ...
    if (!q) return [];
    const scored: Array<{ p: ProblemIndexEntry; s: number }> = [];
...
function ModuleBadge({
  ...
  moduleKey: ProblemIndexEntry["module"];
}) {
