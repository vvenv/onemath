...
import { getKnowledgeBySlug, knowledgeEntries } from "@/data/knowledge";
...
export function AppBar() {
  ...
  const favoriteProblems = favorites
    ...
    .map((f) => getProblemById(f.id))
    .filter((p): p is (typeof problems)[number] => p !== undefined);

  const favoriteKnowledge = favorites
    .filter((f) => f.type === "knowledge")
    .map((f) => getKnowledgeBySlug(f.id))
    .filter((e): e is (typeof knowledgeEntries)[number] => e !== undefined);
