...
function problemSummary(p: ProblemIndexEntry) {
  return {
    id: p.id,
    title: p.title,
    grade: p.grade,
...
function pickRandomProblem(opts?: {
  ...
  useEffect(() => {
    ...
    register({
      ...
      async execute(input) {
        ...
        const filtered = problemsIndex.filter((p) => {
    ...
    register({
      ...
      async execute(input) {
        ...
        // Use direct data access since ssr: false means no lazy loading needed
    ...
    register({
      ...
      async execute(input) {
        ...
        const p = pickRandomProblem({ module });
    ...
    register({
      ...
      async execute(input) {
        ...
        const entries = category
          ? knowledgeIndex.filter((e) => e.category === category)
          : knowledgeIndex;
        return {
    ...
    register({
      ...
      async execute(input) {
        ...
        // Cheap existence check from the slim index avoids loading the full
        // entry just to return a 404.
        if (!getKnowledgeIndexBySlug(slug)) {
          return { error: `knowledge ${slug} not found` };
        }
        // Use direct data access since ssr: false means no lazy loading needed
        const { getKnowledgeBySlug } = await import("@/data/knowledge");
