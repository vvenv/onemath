...
- Do not restate the problem statement inside the figure. Annotations like `BC = 5BD`, `AE : EC = 3 : 1`, `DG = GS = SE` belong in the problem `question`/`caption`, not as inline `<text>` on the drawing. Figures should show geometry; ratios/constraints are conveyed by the question text and solution scenes.
