...
- **Use Tailwind CSS classes for styling**: Prefer `class="stroke-current fill-none"` over inline `stroke="currentColor" fill="none"`. Use `class="fill-current"` instead of `fill="currentColor"`. For color highlighting, use Tailwind classes like `class="fill-red-500/10 stroke-red-500"`.
...
- **Promote common class attributes to parent `<g>`**: When multiple child elements within a group share identical `class` attributes, move the common classes to the parent `<g>` to avoid repetition. For example, instead of `<g><path class="fill-red-500/20 stroke-red-500"/><path class="fill-red-500/20 stroke-red-500"/></g>`, use `<g class="fill-red-500/20 stroke-red-500"><path/><path/></g>`.
- **Text alignment**: Use `text-anchor="middle"` attribute for center-aligned text labels.
- **Do not set explicit font-size or other styling attributes**: Keep SVG minimal and inherit styling from page context. Avoid `font-size`, explicit `width`/`height` on root, and other hard-coded styling that should be inherited.
